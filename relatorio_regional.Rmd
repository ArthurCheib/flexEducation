...
* Um **gráfico** contendo a evolução do número absoluto de turmas no Plano de Atendimento, Criadas e Autorizadas para diferentes datas (as últimas cinco disponíveis no SIMADE).
* Uma **tabela** com dados gerais - por escola - do percentual de turmas criadas e autorizadas e que também se encontra disponível para downlod nos formatos **excel** e **pdf**
...
datatable(tabela_escola_matricula, filter = 'top',  rownames = FALSE,
...
datatable(tabela_escola_criacao, filter = 'top',  rownames = FALSE,
          extensions = 'Buttons', options = list(
                                                 dom = 'Bfrtip',
                                                 buttons = c('excel', 'pdf')),
...
pct_encerrado <- total_encerrados %>%
  ...
  summarize(`TOTAL ENCERRADOS` = sum(`TOTAL ENCERRADOS`),
            TOTAL_ENTURMADOS = sum(`TOTAL ENTURMADOS NO DIA 10/12`)) %>% 
  ungroup() %>% mutate(TOTAL = round((`TOTAL ENCERRADOS`/TOTAL_ENTURMADOS)*100, digits = 2)) %>% .[[4]]
...
```{r}
valueBox(prettyNum(alunos_encerrados, big.mark = "."), color = "brown")
```

### ALUNOS ALUNOS PENDENTES DE ENCERRAMENTO

```{r}
valueBox(prettyNum((alunos_enturmados - alunos_encerrados), big.mark = "."), color = "brown")
```
...
valueBox(prettyNum(escolas_abaixo), color = "blue")
```
